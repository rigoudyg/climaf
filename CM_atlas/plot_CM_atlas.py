...
def plot_climato(var, dat, season, proj='GLOB', domain={}, custom_plot_params={}, do_cfile=True, mpCenterLonF=None,
    ...
    if add_vectors:
        if 'product' in wdat:
            ...
            u_wdat = wdat.copy()
            u_wdat.update(dict(variable=vectors_u))
            v_wdat = wdat.copy()
            v_wdat.update(dict(variable=vectors_v))
            if vectors_grid:
    ...
    if variable in ocean_variables:
        ...
        if 'meshmask_for_navlon_navlat' in wdat:
            climato_dat = add_nav_lon_nav_lat_from_mesh_mask(climato_dat,
                                                             mesh_mask_file=wdat['meshmask_for_navlon_navlat'])
            cdogrid = 'r720x360'
        if not cdogrid:
        ...
        for wref in var_references:
            ...
            for model in models:
                ...
                model_diff = plot_diff(var, wmodel, ref, season, proj=proj, domain=domain,
                                       ...
                                       do_cfile=do_cfile)  # , remapping=remapping)
                if do_cfile:
