...
parser.add_argument('--highlight_period', action='store', default=None, help='Highlight a period on a time series (thicker line) ; provide the periods yearstart_yearend separated by commas (Ex: 1980_2005,1990_2000 to highlight the first period on the first dataset, and the second period on the second dataset)')
...
parser.add_argument('--xlim', action='store', default='', help='Provide the start date and end date to force the X axis. Ex: 1950-01-01,2005-12-31')
...
parser.add_argument('--text', action='store', default="", help='add some text in the plot; the user provides a triplet separared with commas x,y,text; separate the triplets with | if you want to provide multiple texts. Ex: x1,y1,text1|x2,y2,text2')
parser.add_argument('--text_fontsize', action='store', default="", help='fontsize of the text (separate with commas if provide several')
parser.add_argument('--text_colors', action='store', default="", help='color of the text (separate with commas if provide several')
parser.add_argument('--text_verticalalignment', action='store', default="", help='Vertical alignment of the text (separate with commas if provide several')
parser.add_argument('--text_horizontalalignment', action='store', default="", help='Horizontal alignment of the text (separate with commas if provide several')
...
parser.add_argument('--legend_xy_pos', action='store', default=None, help='x,y Position of the corner of the box (by default = upper left corner). Example= "1.02,1"')
parser.add_argument('--legend_loc', action='store', default=None, help='Choose the corner of the legend box to specify the position of the legend box; by default 2 (upper left corner), take values 1, 2, 3 or 4 (see resource loc of pyplot legend)')
...
if args.min: mini=float(args.min)
if args.max: maxi=float(args.max)
...
if args.highlight_period:
    ...
    if not len(filenames_list)==len(highlight_period_list):
        print 'Provided ',len(filenames_list),' time series and only ',len(highlight_period_list),' periods to highlight'
...
if args.horizontal_lines_values:
    ...
    if len(horizontal_lines_lw_list)==1 and len(horizontal_lines_values_list)>1: horizontal_lines_lw_list = horizontal_lines_lw_list * len(horizontal_lines_values_list)
    ...
    if len(horizontal_lines_colors_list)==1 and len(horizontal_lines_values_list)>1: horizontal_lines_colors_list = horizontal_lines_colors_list * len(horizontal_lines_values_list)
...
for pathfilename in filenames_list:
    ...
    print('time_test :', time_test[0])
    print('time_test :', np.shape(time_test))
    print('time_test :', type(time_test[0]))
    print('X :', x[0])
    print('X :', np.shape(x))
    print('X :', type(x[0]))
    print('Y :', int(y[0]))
    ...
    if args.highlight_period:
        ...
        endyear = int(dum[1])
        #
        ind = np.argwhere((datevar[0]>datetime.datetime(startyear, 1, 1)) & (datevar[0]<datetime.datetime(endyear, 12, 31))).flatten()
        print 'highlight_period = ',highlight_period
        #plt.plot(x[ind],y[ind],lw=highlight_period_lw_list[filenames_list.index(pathfilename)],
        #         color=colors[filenames_list.index(pathfilename)])
   ...
   for xlim_date in [xlim_start_date,xlim_end_date]:
       if len(xlim_date)==4:
          ...
          if '-' in x_text or '_' in x_text:
             ...
             if len(split_x)==2: x_date = datetime.datetime(int(split_x[0]), int(split_x[1]))
             if len(split_x)==3: x_date = datetime.datetime(int(split_x[0]), int(split_x[1]), int(split_x[2]))
          elif len(x_text)==6:
...
# -- Add the titles
if args.left_string: plt.title(args.left_string, loc='left', fontsize=(float(args.left_string_fontsize) if args.left_string_fontsize else default_left_string_fontsize ))

if args.center_string: plt.title(args.center_string, loc='center', fontsize=(float(args.center_string_fontsize) if args.center_string_fontsize else default_center_string_fontsize ))

right_string_fontsize = (float(args.right_string_fontsize) if args.right_string_fontsize else default_right_string_fontsize )
...
# -- X and Y axis labels
if args.xlabel: plt.xlabel(args.xlabel,fontsize=(float(args.xlabel_fontsize) if args.xlabel_fontsize else default_xlabel_fontsize ))
if args.ylabel: plt.ylabel(args.ylabel,fontsize=(float(args.ylabel_fontsize) if args.ylabel_fontsize else default_ylabel_fontsize ))
...
if args.text:
    ...
    if len(text_fontsize_list)==1 and len(text_list)>1: text_fontsize_list = text_fontsize_list * len(text_list)
    text_colors_list = (str.split(args.text_colors,',') if args.text_colors else ['black']*len(text_list) )
    if len(text_colors_list)==1 and len(text_list)>1: text_colors_list = text_colors_list * len(text_list)
    text_verticalalignment_list= (str.split(args.text_verticalalignment,',') if args.text_verticalalignment else ['bottom']*len(text_list) )
    if len(text_verticalalignment_list)==1 and len(text_list)>1: text_verticalalignment_list = text_verticalalignment_list * len(text_list)
    text_horizontalalignment_list= (str.split(args.text_horizontalalignment,',') if args.text_horizontalalignment else ['left']*len(text_list) )
    if len(text_horizontalalignment_list)==1 and len(text_list)>1: text_horizontalalignment_list = text_horizontalalignment_list * len(text_list)
