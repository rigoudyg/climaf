...
def cmip6_optimize_wildcards(kwargs):
    ...
    for inst in institutes:
        paths.extend(listdirs(root+"/"+mip, inst, test_exists = wild(model)))
    ...
    for path in paths:
        ...
        models = possible_values("CMIP6","mip_institute_experiment2model", root, "%s_%s_%s_"%(mip,institute,experiment), model)
        # if wild(experiment):
        #     institute = cmip6_facets(path, root, 2)[0]
        #     models = possible_values("CMIP6","mip_institute2model", root, "%s_%s_"%(mip,institute), model)
        # else: 
        #     models = possible_values("CMIP6","mip_experiment2model", root, "%s_%s_"%(mip,experiment), model)
    ...
    for path in paths:
        ...
        experiments = possible_values("CMIP6","mip_model2experiment", root, "%s_%s_"%(mip,model), experiment)
    ...
    for path in paths:
        ...
        realizations = possible_values("CMIP6","mip_model_experiment2realization",\
                                       root, "%s_%s_%s_"%(mip,model,experiment), realization)
    ...
    if wild(table):
        ...
        tables = ['3hr', '6hrLev', '6hrPlev', '6hrPlevPt', 'AERday', 'AERhr', 'AERmon', 'AERmonZ', 'Amon', 'CF3hr', 'CFday', 'CFmon', 'CFsubhr', 'day', 'E1hr', 'E3hr', 'Eday', 'EdayZ', 'Efx', 'Emon', 'EmonZ', 'fx', 'LImon', 'Lmon', 'Oday', 'Ofx', 'Omon', 'SIday', 'SImon']
...
def dirnames_for_one_case(case_name, glob_pattern, split_index, case_value,
    ...
    #
    if case_name not in dirnames:
        dirnames[case_name] = dict()
        should_write = True
        # Build table by globbing
        clogger.warning("Building table %s by globbing. "%(case_name)+\
          "\n\tThis may take a while but will be saved for further sessions. "+\
          "\n\tTable will be stored in the cache as %s"%filen) 
        values = set()
        t = time.time()
        ...
        for case in cases:
            if not isinstance(key_index,list):
                ...
                for k in key_index:
                    key += "%s_"%(case.split("/")[k])
            if key not in dirnames[case_name]:
...
def possible_values(project, tag, root, key, value_pattern):
    ...
    params = {
        "CMIP6" : {
            "experiment2mip"       : { "glob_pattern": "/*/*/*/*", "split_index": -4, "key_index": -1 },
            "mip_institute2model"  : { "glob_pattern": "/*/*/*"  , "split_index": -1, "key_index": [-3,-2] },
            "model2institute"      : { "glob_pattern": "/*/*/*"  , "split_index": -2, "key_index": -1 },
            "mip_experiment2model" : { "glob_pattern": "/*/*/*/*", "split_index": -2, "key_index": [-4,-1] },
            "mip_institute_experiment2model" : { "glob_pattern": "/*/*/*/*", "split_index": -2, "key_index": [-4,-3,-1] },
    ...
    broot = root.encode('utf-8')
    root_tag = hashlib.sha1(broot).hexdigest()[0:8]
    case_name = "%s_%s_%s"%(project,tag,root_tag)
    #
    paras = None
    if tag in params[project] :
        paras = params[project][tag]
    else:
        for a_tag in params[project] :
            if a_tag in tag :
