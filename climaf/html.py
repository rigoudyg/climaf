...
def link(label,filename,thumbnail=None,hover=True) :
    ...
    this image when you mouse over it (with the provided hover size).
    ...
    - 'thumbnail' can be an integer or a string with width or height
      ...
      separated by character 'x' or '*'. The size is in pixels; default=None 
    - 'hover' can be a logical, or a string with width or height (in this
      ...
      character 'x' or '*'. The size is in pixels; default=True, i.e. :

      - if thumbnail is not None, hover width and height are respectively
        set in 3 times that of thumbnail width and height
      - if thumbnail is None, hover=True corresponding to '200*200' 
    """
    if filename :
...
def cell(label,filename=None,thumbnail=None,hover=True,dirname=None) :
    ...
    If 'dirname' is not None, create a directory (if doesn't exist) wich
    contains the provided filename as a hard link to the target
    dirname/'climaf_atlas'([0-9]+).ext ('ext' is 'png', 'pdf' or 'eps');
    this allows to save disk space, but does not allow to cross filesystem
    boundaries. 'dirname' can have a relative or absolute path, as long as
...
def line(dic,title="",thumbnail=None,hover=True,dirname=None):
    ...
    can also be a list of filenames. If 'dirname' is not None, create
    a directory (if doesn't exist) wich contains the provided filenames
    as hard links to the targets dirname/'climaf_atlas'([0-9]+).ext
    ('ext' is 'png', 'pdf' or 'eps')
    """
    if thumbnail :
