...
General:
  ...
  - ``options``, ``aux_options``, ``shading_options`` : strings for
    setting NCL graphic resources defined by user for different
    fields, each resource separated by "|". This lists are priority in
    front of graphic resources in the script. Each field has an
    specific argument, e.g. :  
