...
- V3.x:
  ...
  - Add operator **plotmap**, a replacement for operator `plot`, albeit only for maps. It is based on Matplotlib, Cartopy and GeoCat Viz; see :doc:`scripts/plotmap`. Calls to plot that end up as a map should be transformed accordingly. You can also set `env.environment.plot_use_plotmap=True` for automatically use plotmap, but this will fail when plot is used for 1- and 2-d profiles, curves or time series. If then setting `env.environment.plot_use_plotmap = True`, the generated plotmap call will be displayed.
